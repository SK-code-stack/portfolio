  ...
  return (
    <>
      <div className="mb-16 pt-20 -mt-20  transition-all duration-1000" id='projects'>
              <h1 className={` mb-16 font-custom  bg-gradient-to-b text-5xl flex justify-center ${
              darkMode ? 'from-zinc-200 via-zinc-400 to-zinc-50' : 'from-zinc-300 via-zinc-900 to-zinc-300'
            } bg-clip-text font-light tracking-wide text-transparent transition-all duration-1000`}>My<h1 className='color_animate '>&nbsp;Work</h1></h1>
        
        {/* Project cards grid */}
        <div className="container p-3 mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 transition-all duration-1000">
          {projects.map((project) => (
            <div className="myanimation  p-1 rounded-lg ">
            <div 
            key={project.id}
            className=" bg-custom-mylight text-custom-mydark dark:bg-custom-myblack dark:text-custom-mylight h-full rounded-lg overflow-hidden myshadow  hover:scale-105 font-custom transition-all duration-1000"
            >
        ...
        <div className="flex justify-center p-5">
            <a href="#project" className="bg-custom-mylight text-custom-myblack hover:bg-blue-600 dark:hover:bg-blue-600 dark:bg-custom-myblack dark:text-custom-mylight px-2 py-1 rounded myshadow  active:scale-95 transition-all duration-1000">see more</a>
        </div>
      </div>
    </>
  );
